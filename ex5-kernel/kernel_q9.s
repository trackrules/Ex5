...
main:   
    ...
    sw $1, pcb_link($1)         #Setup the link field 

    la $2, idle_main          #Setup the Sear field
    sw $2, pcb_ear ($1)
...
exitclean:
    sw $0, exitcount($0)
    la $1, idle_pcb         #load idle pcb, and call dispatcher
    sw $1, current_task($0) 


idle_main:				#entry point into the program
...
loop:
	...
	bnez $8, idle_main		#if so, go to "idle_main"
...
time_slice:
    .word 2
hello:				#SSD patterns to encode "HELLO C200"
	.word 0x76 #H
	.word 0x79 #E
	.word 0x38 #L
	.word 0x38 #L
